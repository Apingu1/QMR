...
def excel_date(v):
    try:
        ...
        if d.time()==datetime.min.time(): return d.strftime('%Y-%m-%d')
    ...
    with zipfile.ZipFile(path) as z:
        ...
        for s in wb.findall('.//m:sheets/m:sheet',NS):
            ...
            for c in root.findall('.//m:sheetData/m:row/m:c',NS):
                ref=c.attrib.get('r',''); row=int(re.search(r'(\d+)$',ref).group(1)); col=col_num(ref); maxc=max(maxc,col); maxr=max(maxr,row)
                t=c.attrib.get('t'); style=int(c.attrib.get('s','0')); v=c.find('m:v',NS); inline=c.find('m:is',NS); f=c.find('m:f',NS)
                val=''
                if t=='s' and v is not None:
                    try: val=shared[int(v.text)]
                    except: val=v.text or ''
                elif t=='inlineStr' and inline is not None:
                    val=''.join(x.text or '' for x in inline.iterfind('.//m:t',NS))
...
for p in sorted((root/'0. QMS Spreadsheets 2026').glob('*.xlsx'))+[root/'Root Cause Analysis Data & Trending (Version 2).xlsx']:
    data=parse_xlsx(p)
    (outdir/(p.stem+'.json')).write_text(json.dumps(data,indent=2,ensure_ascii=False),encoding='utf-8')
...
print('extracted', len(list(outdir.glob('*.json'))),'files')
